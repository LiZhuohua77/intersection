...
def resample_path(path, segment_length=1.0):
    """
    对路径进行等距重采样，生成点间距离恒定的路径。
    
    Args:
        path (list): 路径点列表 [[x1, y1], [x2, y2], ...]
        segment_length (float): 重采样后相邻点之间的目标距离

    Returns:
        list: 重采样后的等距路径点列表 [[x1, y1], [x2, y2], ...]
    """
    path = np.array(path)
    # 计算沿路径的累积距离
...
def recalculate_angles(path_points):
    ...
    Args:
        path_points (list): 路径点列表 [[x1, y1], [x2, y2], ...]
        
    Returns:
        list: 包含位置和朝向角的三元组列表 [(x1, y1, angle1), (x2, y2, angle2), ...]
              其中angle为弧度制，表示该点处路径的切线方向（朝向）
    """
    points_with_angle = []
