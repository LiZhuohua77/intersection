...
VEHICLE_LF = 1.2     # 前轴到质心距离 (m)
VEHICLE_LR = 1.6     # 后轴到质心距离 (m)
VEHICLE_W = 2.0      # 车辆宽度 (m)
VEHICLE_L = 4.7      # 车辆长度 (m)
...
MPC_Q = [100.0, 100.0]     # 状态误差权重 [y_e, psi_e]
MPC_R = [0.1]            # 控制输入权重 [delta]
MPC_RD = [10.0]          # 控制输入变化率权重 [delta_dot]
MAX_STEER_ANGLE = np.deg2rad(30.0) # 最大方向盘转角

# Gipps 模型参数
GIPPS_A = 1.8               # m/s^2, 期望加速度
GIPPS_B = -3.5              # m/s^2, 期望减速度
GIPPS_V_DESIRED = 10.0      # m/s, 期望速度 (~54 km/h)
